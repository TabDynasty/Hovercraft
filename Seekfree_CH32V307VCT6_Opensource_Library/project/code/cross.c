...
void check_Cross()
{
    ...
    else if(Lpt1_found&&Lpt1_rpts1s_id<RcrossBegin_thred&&cross_type==CROSS_NONE&&!is_straight0&&rpts0s_num>5)
        {
            check_Right_Cross();
...
void run_Cross()
{
    switch (cross_type)
    {
        ...
        case CROSS_IN:
            check_Left_Cross();
            check_Right_Cross();
            static bool begin_y_flag=0;
            if(begin_y_flag==0)
            {
                begin_y-=cross_begin_y;
                begin_y_flag=1;
            }
...
void check_Right_Cross()
{
    ...
    else if(circle_type==0 && far_Lpt1_found)//右远L角点找到,则向右下偏移作为寻找原线的起始点
    {
        ...
        inv_Back_Lpt1[0]=Cal_inv_rot_x((far_rpts1s[clip(far_Lpt1_rpts1s_id+back_Position,0,far_rpts1s_num-1)][0]),far_rpts1s[clip(far_Lpt1_rpts1s_id+back_Position,0,far_rpts1s_num-1)][1]);
